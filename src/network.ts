...
for (const name of actionTypeNames) {
	...
	if (byteLength > TRYSTERO_ACTION_NAME_MAX_BYTES) {
		throw new Error(
			`Trystero action type "${name}" is ${byteLength} bytes; must be <= ${TRYSTERO_ACTION_NAME_MAX_BYTES}`,
		);
	}
}
...
export function useOnlineSession(enabled: boolean, roomId = DEFAULT_ROOM_ID): OnlineSession {
	...
	useEffect(() => {
		...
		let sendSnapshotRequest:
			| ((message: SnapshotRequestMessage, target?: string | string[] | null) => Promise<void[]>)
			| null = null;
		let sendPlayerAction:
			| ((message: PlayerActionMessage, target?: string | string[] | null) => Promise<void[]>)
			| null = null;
		...
		const updateHostedSnapshot = (mutate: (draft: RoomSnapshot) => void): void => {
			...
			const base =
				hostedSnapshot ??
				currentSnapshot ??
				createInitialSnapshot(selfId, peerNames.get(selfId) ?? formatPeerName(selfId));
		...
		const refreshHostedMembers = (): void => {
			...
			const members = withResolvedMemberSeats(baseline.phase, connectedMembers, baseline.members, baseline.gameState);
		...
		const becomeHost = (): void => {
			...
			const base = cloneSnapshot(
				currentSnapshot ?? createInitialSnapshot(selfId, peerNames.get(selfId) ?? formatPeerName(selfId)),
			);
			const connected = getConnectedPeerIds(selfId, room);
			const previousMembers = base.members;
		...
		const initialize = async (): Promise<void> => {
			...
			const [sendRequestSnapshotAction, getRequestSnapshotAction] =
				room.makeAction<SnapshotRequestMessage>(REQUEST_SNAPSHOT_NAMESPACE);
