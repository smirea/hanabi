...
describe('networkShared', () => {
	test('normalizeSettings enforces multicolor constraints', () => {
		expect(
			normalizeSettings({ includeMulticolor: false, multicolorShortDeck: true, multicolorWildHints: true }),
		).toEqual({
		...
		expect(
			normalizeSettings({ includeMulticolor: true, multicolorShortDeck: true, multicolorWildHints: true }),
		).toEqual({
		...
		expect(
			normalizeSettings({ includeMulticolor: true, multicolorShortDeck: true, multicolorWildHints: false }),
		).toEqual({
			includeMulticolor: true,
			multicolorShortDeck: true,
			multicolorWildHints: true,
	...
	test('applyNetworkAction rejects actions from non-current players', () => {
		...
		const wrongActor = game.state.players.find(
			player => player.id !== game.state.players[game.state.currentTurnPlayerIndex]?.id,
		)?.id;
