...
export function useRoomDirectoryListing(enabled: boolean): {
	...
	const rooms = useMemo(() => {
		void version;
		return [...roomsRef.current.values()].map(entry => entry.listing).sort((a, b) => b.updatedAt - a.updatedAt);
	}, [version]);
...
export function useRoomDirectoryAdvertiser({
	...
	useEffect(() => {
		...
		const sendCurrent = (
			send: ((message: DirectoryAnnouncement, target?: string | string[] | null) => Promise<void[]>) | null,
		): void => {
			if (!send) {
				return;
			}
