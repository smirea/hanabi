...
export function TvScreen({
	...
	const hintTokenStates = Array.from(
		{ length: gameState.settings.maxHintTokens },
		(_, index) => index < gameState.hintTokens,
	);
	...
	const { knownUnavailableCounts, knownRemainingCounts } = useMemo(() => {
		...
		for (const suit of activeSuits) {
			for (const number of CARD_NUMBERS) {
				const totalCopies = suit === 'M' && gameState.settings.multicolorShortDeck ? 1 : (copiesByNumber[number] ?? 0);
			...
			<section className='tv-stage'>
				<header className='tv-header'>
					<div className='tv-header-title'>
						<h1 className='tv-title' data-testid='tv-title'>
							TV Mode
						</h1>
						<div className='tv-meta'>
							<span className='tv-meta-item' data-testid='tv-score'>
								Score {score}
							</span>
							<span className='tv-meta-item' data-testid='tv-turn'>
								Turn {gameState.turn}
							</span>
							{currentTurnPlayer && (
								<span className='tv-meta-item' data-testid='tv-current-player'>
				...
				<section className='stats tv-stats' data-testid='tv-stats'>
					<div className='stat hints-stat' data-testid='tv-status-hints'>
						...
						<span className='visually-hidden' data-testid='tv-status-hints-count'>
					...
					<div className='stat deck-stat' data-testid='tv-status-deck'>
						<div className='deck-pill'>
							...
							<span className='deck-count' data-testid='tv-status-deck-count'>
								{gameState.drawDeck.length}
							</span>
						</div>
					</div>
					...
					<div className='stat fuses-stat' data-testid='tv-status-fuses'>
						...
						<span className='visually-hidden' data-testid='tv-status-fuses-count'>
							{remainingFuses}
						</span>
					</div>
				</section>
							...
							>
								<div className='tower-stack'>
									{CARD_NUMBERS.map(num => {
										...
										const pipStates = getPegPipStates('default', remaining, 0, discarded, played, pipTotal);
										...
										return (
											...
											>
												...
												<span
													className='peg-pips'
													aria-label={`${remaining} available, ${discarded + played} unavailable`}
												>
