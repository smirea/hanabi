...
export function useGameAnimations({
	...
	const playerNameFxListenerByNodeRef = useRef<WeakMap<HTMLSpanElement, (event: AnimationEvent) => void>>(
		new WeakMap(),
	);
	const turnAudioContextRef = useRef<AudioContext | null>(null);
	const cardFxListenerByNodeRef = useRef<WeakMap<HTMLButtonElement, Map<string, (event: AnimationEvent) => void>>>(
		new WeakMap(),
	);
	...
	const triggerCardFx = useCallback((cardId: CardId, fxClass: string): void => {
		...
		const expectedAnimationName =
			fxClass === 'hint-enter' ? 'hint-ring' : fxClass === 'hint-redundant' ? 'hint-redundant-ring' : null;
	...
	async function animateDrawCard({
		...
		try {
			...
			await animate(destNode, { opacity: [0, 1], scale: [0.98, 1] }, { duration: 0.16, ease: [0.2, 0.85, 0.2, 1] })
				.finished;
		} finally {
			ghost.root.remove();
	...
	useEffect(() => {
		...
		const canRunMotion =
			!reduceMotion && typeof document !== 'undefined' && typeof document.createElement('div').animate === 'function';
		if (!canRunMotion) {
			return;
		}
