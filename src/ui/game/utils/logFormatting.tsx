...
export function renderLogMessage(log: GameLogEntry): ReactNode {
	if (log.type === 'hint') {
		...
		if (log.hintType === 'number') {
			...
			return (
				<>
					{log.actorName} hinted {touchedCount}x <LogHintChipNumber number={log.number} /> to {log.targetName}
				</>
			);
		}
		...
		return (
			<>
				{log.actorName} hinted {touchedCount}x <LogHintChipSuit suit={log.suit} /> to {log.targetName}
			</>
		);
	}
