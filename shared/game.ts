...
export class HanabiGame {
	...
	public getPerspectiveState(viewerId: PlayerId): HanabiPerspectiveState {
		...
		for (const suit of ALL_SUITS) {
			for (const number of CARD_NUMBERS) {
				...
				knownRemainingCounts[suit][number] = Math.max(
					0,
					totalCopies - knownUnavailableCounts[suit][number],
				);
			}
		}
	...
	public giveNumberHint(targetPlayerId: PlayerId, number: CardNumber): void {
		...
		const touchedCardIds = targetPlayer.cards.filter(
			cardId => this.state.cards[cardId].number === number,
		);
		if (touchedCardIds.length === 0) {
	...
	private areAllFireworksComplete(): boolean {
		return this.state.settings.activeSuits.every(
			suit => this.state.fireworks[suit].length === CARD_NUMBERS.length,
		);
	}
