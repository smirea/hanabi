...
export function LobbyScreen({
	...
	const effectiveMembers = selfPlayerId
		? members.map(member => (member.id === selfPlayerId ? { ...member, isTv: selfIsTv } : member))
		: members;
	...
	const playerCountError =
		seatedCount > 5 ? 'Max 5 players' : seatedCount < 2 ? 'Need at least 2 players' : null;
			...
			<section className='lobby-shell-body lobby-shell-body-full'>
				<section className='lobby-card'>
					...
					<div className='lobby-info-row'>
						<label className='lobby-info-row-label' htmlFor='lobby-name-input'>
							Name
						</label>
						<input
							id='lobby-name-input'
					...
					<section className='lobby-players'>
						...
						<div className='lobby-player-list'>
							{effectiveMembers.map(member => (
								<article
									key={member.id}
									className={`lobby-player${member.id === selfPlayerId ? ' self' : ''}`}
									data-testid={`lobby-player-${member.id}`}
								>
									<div className='lobby-player-name'>{member.name}</div>
									<div className='lobby-chip-row'>
										{member.id === selfPlayerId && <span className='lobby-chip'>You</span>}
										{member.isReady && !member.isTv && (
											<span className='lobby-chip ready'>Ready</span>
										)}
					...
					<section className='lobby-actions'>
						...
						{phase === 'lobby' ? (
							...
							<p className='lobby-waiting lobby-action-main' data-testid='lobby-waiting-consensus'>
								Game in progress.
							</p>
						)}
						<button
