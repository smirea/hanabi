...
export function getPegPipStates(
	...
	if (mode === 'tibi') {
		...
		const clampedHand = Math.min(
			Math.max(visibleCount, 0),
			Math.max(0, clampedTotal - clampedDeck),
		);
	...
	const clampedUnavailable = Math.min(
		unavailableCount,
		Math.max(0, clampedTotal - clampedAvailable),
	);
		...
		<>
			{[...visible].reverse().map(pip => (
				<span key={`pip-${pip.index}`} className={`peg-pip ${pip.state}`} aria-hidden>
					{pip.state === 'deck' && (
						<CardsThree size={9} weight='bold' className='peg-pip-icon' aria-hidden />
					)}
					{pip.state === 'hand' && (
