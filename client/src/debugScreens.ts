...
}: BuildMockLogsInput): GameLogEntry[] {
	const [p1, p2, p3] = MOCK_PLAYERS.ids;
	const [n1, n2, n3] = MOCK_PLAYERS.names;
	const touchedTwo: CardId[] = [
		cardSamplesBySuitNumber.R1 ?? 'c001',
		cardSamplesBySuitNumber.Y1 ?? 'c002',
	];
...
function createWinState(): HanabiState {
	...
	for (const suit of BASE_SUITS) {
		for (const number of CARD_NUMBERS) {
			const found = Object.entries(state.cards).find(
...
function createGameState(): HanabiState {
	...
	const setHint = (
		...
		hint: Partial<HanabiState['cards'][CardId]['hints']>,
	): void => {
		if (!cardId) return;
...
function getMockState(screen: DebugScreenName): HanabiState {
	...
	const next =
		screen === 'win' ? createWinState() : screen === 'lose' ? createLoseState() : createGameState();
...
function dispatchDebugScreen(screen: DebugScreenName): void {
	...
	window.dispatchEvent(
		new CustomEvent<DebugScreenEventDetail>(DEBUG_SCREEN_EVENT, {
			detail: { screen, state },
		}),
	);
}
