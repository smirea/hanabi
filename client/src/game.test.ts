...
function twoPlayerDeck(
	playerOneHand: DeckCard[],
	playerTwoHand: DeckCard[],
	tail: DeckCard[] = [],
): DeckCard[] {
...
describe('HanabiGame', () => {
	...
	test('discard is always allowed and only regains a hint when not already maxed', () => {
		...
		expect(game.state.players.find(player => player.id === currentPlayer.id)?.cards).toHaveLength(
			5,
		);
	...
	test('selection API rejects invalid contexts and missing confirmation data', () => {
		...
		expect(() => game.selectHintTarget('p2')).toThrow(
			'Hint target selection is only available for hint actions',
		);
	...
	test('multicolor deck is always short-mode when enabled', () => {
		...
		const defaultMCards = Object.values(wildHintMulticolorByDefault.state.cards).filter(
			entry => entry.suit === 'M',
		);
		...
		expect(
			Object.values(requestedLongMulticolor.state.cards).filter(entry => entry.suit === 'M'),
		).toHaveLength(5);
		...
		expect(
			Object.values(shortWildMulticolor.state.cards).filter(entry => entry.suit === 'M'),
		).toHaveLength(5);
	});
	...
	test('rejects invalid hint targets and hint feasibility rules', () => {
		...
		expect(() => noTokenGame.giveColorHint('p2', 'R')).toThrow(
			'Cannot give a hint with zero hint tokens',
		);
	});
	...
	test('number hint selection highlights only matching number cards', () => {
		...
		const expected = game.state.players[1].cards.filter(
			cardId => game.state.cards[cardId].number === 2,
		);
	...
	test('card selection only accepts cards from current player hand', () => {
		...
		expect(() => game.selectCard(otherPlayerCard)).toThrow(
			'Selected card is not in the current player hand',
		);
	});
	...
	test('recentlyHinted flags are cleared by subsequent actions', () => {
		...
		game.giveNumberHint('p2', 2);
		const initiallyHinted = game.state.players[1].cards.filter(
			cardId => game.state.cards[cardId].number === 2,
		);
	...
	test('redundant hints are rejected and do not duplicate exclusions', () => {
		...
		const targetCard = game.state.players[1].cards.find(
			cardId => game.state.cards[cardId].suit !== 'R',
		)!;
		game.giveColorHint('p2', 'R');
		game.giveNumberHint('p1', 1);
		...
		expect(game.state.players[game.state.currentTurnPlayerIndex]?.id ?? null).toBe(
			currentPlayerBefore,
		);

		const exclusionsAfter = game.state.cards[targetCard].hints.notColors.filter(
			value => value === 'R',
		);
	...
	test('touched hint removes stale exclusions imported via snapshot', () => {
		...
		const targetCardId = game.state.players[1].cards.find(
			cardId => game.state.cards[cardId].suit === 'G',
		)!;
	...
	test('turn automatically skips players who have no legal actions', () => {
		...
		const game = HanabiGame.fromState(state);
		const cardId = game.state.players[0].cards.find(
			candidate => game.state.cards[candidate].number !== 5,
		)!;
		game.playCard(cardId);
	...
	test('perspective view rejects unknown viewer ids', () => {
		...
		expect(() => game.getPerspectiveState('missing')).toThrow(
			'Unknown perspective player: missing',
		);
	});
	...
	test('multicolor color hints treat multicolor cards as matching any called base color', () => {
		...
		const targetCards = game.state.players[1].cards;
		const expectedHighlighted = targetCards.filter(cardId =>
			['R', 'M'].includes(game.state.cards[cardId].suit),
		);
		expect(game.state.ui.highlightedCardIds).toEqual(expectedHighlighted);
		...
		expect(
			targetCards.every(cardId => !game.state.cards[cardId].hints.notColors.includes('M')),
		).toBeTrue();
